...
contract Transfer {
    ...
    function transfer(string memory destChainServiceID, string memory sender, string memory receiver, uint64 amount) public {
        ...
        // argsRb[0] is rollback length flag
        // argsRb[1:] is rollback args
        bytes[] memory argsRb = new bytes[](3);
        // set rollback arg length
        argsRb[0] = abi.encodePacked(uint64(2));
        argsRb[1] = abi.encodePacked(sender);
        argsRb[2] = abi.encodePacked(amount);
        string[] memory func = new string[](3);
        func[0] = "interchainMultiCharge";
        func[1] = "";
        func[2] = "interchainMultiRollback";
    ...
    function multiTransfer(string memory destChainServiceID, string[] memory sender, string[] memory receiver, uint64[] memory amount) public {
        ...
        // args[0]: specify single or multi, single:0; multi:1
        // args[1]: specify every single args length
    ...
    function transferOne2Multi(string[] memory destChainServiceIDs, string[] memory senders, string[] memory receivers, uint64[] memory amounts) public {
        ...
        for (uint i = 0; i < len; i++)
        {
            ...
            Broker(BrokerAddr).emitInterchainEvent(destChainServiceIDs[i], "interchainMultiCharge", args, "", new bytes[](0), "interchainMultiRollback", argsRb, false, group);
        }
    }
...
//    }
    ...
    function interchainMultiRollback(bytes[] memory args, bool[] memory multiStatus) public onlyBroker {
        uint64 arglenth = bytesToUint64(2);
        string memory sender;
        uint64 amount;
        if (multiStatus.length == 0){
            multiStatus = new bool[]((args.length-1)/2);
