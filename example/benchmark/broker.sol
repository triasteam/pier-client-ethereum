...
contract Broker {
        ...
        bool[] memory isEncrypt) payable external
    {
        for (uint8 i = 0; i <  srcFullID.length; ++i) {
            ...
            enAndBatch[1] = true;
            bytes[][] memory multiArgs = new bytes[][](1);
            multiArgs[0]=args[i];
            invokeInterchainWithBatchFlag(srcFullID[i], destAddr[i], index[i], typ[i], callFunc[i], multiArgs, txStatus[i], signatures[i], enAndBatch);        }
    }
        ...
        bool isEncrypt) payable public
    {
        ...
        bytes[][] memory multiArgs = new bytes[][](1);
        multiArgs[0]=args;
        invokeInterchainWithBatchFlag(srcFullID, destAddr, index, typ, callFunc, multiArgs, txStatus, signatures, enAndBatch);
    }
    ...
    // 地址变为string格式，这样多签不会有问题，在验证多签之前使用checksum之前的合约地址
        ...
        {
            bool ok = brokerData.checkMultiInterchainMultiSigns(srcFullID, dstFullID, index, typ, callFunc, multiArgs, txStatus, signatures, validators, valThreshold);
            if (!ok) {
                invokeIndexUpdateWithError(srcFullID, dstFullID, index, txStatus, enAndBatch[0], "invalid Interchain-multi-signature", uint64(multiArgs.length));
                return;
            }

            if (localWhiteList[brokerData.stringToAddress(destAddr)] == false) {
                invokeIndexUpdateWithError(srcFullID, dstFullID, index, txStatus, enAndBatch[0], "dest address is not in local white list", uint64(multiArgs.length));
                return;
            }
        }

        bool[] memory multiStatus = new bool[](multiArgs.length);
        bytes[][] memory results = new bytes[][](multiArgs.length);
        typ = 1;
        if (txStatus == 0) {
            if (!enAndBatch[1]) {
                if (brokerData.getInCounter(servicePair) < index) {
                    (multiStatus, results) = callMultiService(brokerData.stringToAddress(destAddr), callFunc, multiArgs, false);
            ...
            if (brokerData.getInCounter(servicePair) >= index) {
                (multiStatus, results) = callMultiService(brokerData.stringToAddress(destAddr), callFunc, multiArgs, true);
            }
    ...
    function invokeReceipts(
        ...
        for (uint8 i = 0; i < srcAddrs.length; ++i) {
            bool[] memory multiStatus = new bool[](1);
            //0: interchain; 1: receipt_success; 2: receipt_fail; 3: receipt_Rollback
            if (typs[i] == 1) {
                multiStatus[0]= true;
            }
            // batch flag is true
            invokeReceiptWithBatchFlag(srcAddrs[i], dstFullIDs[i], indexs[i], typs[i], batchResults[i], batchTxStatus[i], batchSignatures[i], multiStatus, true);
        }
    }
        ...
        bytes[] memory signatures) payable public
    {
        ...
        // batch flag is false
        invokeReceiptWithBatchFlag(srcAddr, dstFullID, index, typ, results, txStatus, signatures, multiStatus, false);
    }
        ...
        bool batch) internal
    {
        ...
        }
        {
            ...
            require(brokerData.checkReceiptMultiSigns(srcFullID, dstFullID, index, typ, results, txStatus, signatures, validators, valThreshold));
        }
